...
contract xERC4626Test is DSTestPlus {
    ...
    function testTotalAssetsDuringRewardDistribution(
        uint128 seed,
        uint128 reward
    ) public {
        ...
        unchecked {
            hevm.assume(
                seed != 0 && reward != 0 && combined < type(uint128).max
            );
        }
    ...
    /// @dev test totalAssets call before, during, and after a reward distribution that starts on cycle start
    function testTotalAssetsDuringDelayedRewardDistribution(
        uint128 seed,
        uint128 reward
    ) public {
        uint256 combined = uint256(seed) + uint256(reward);
    ...
    function testTotalAssetsAfterDeposit(uint128 deposit1, uint128 deposit2)
        public
    {
    ...
    function testTotalAssetsAfterWithdraw(uint128 deposit, uint128 withdraw)
        public
    {
    ...
    function testSyncRewardsAfterEmptyCycle(uint128 seed, uint128 reward)
        public
    {
        uint256 combined = uint256(seed) + uint256(reward);
